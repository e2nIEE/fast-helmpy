...
def algorithm_str(DSB_model, aply_DSB_model):
    """Construct algorithm string"""
    algorithm = 'NR'
    if DSB_model:
        algorithm = algorithm + ' DS '
        if aply_DSB_model:
            algorithm = algorithm + 'DSB_model=True'
        else:
            algorithm = algorithm + 'DSB_model=False'
    return algorithm
...
def run_helmpy_nr_functions(detailed_print, cases_to_test, methods):
    ...
    for DSB_model, aply_DSB_model in methods:
        if detailed_print:
            print("\n##########   " + algorithm_str(DSB_model, aply_DSB_model) + '   ##########')
        for case in cases_to_test:
            if detailed_print:
                print("\nCase: " + case.name)
            
            scale = 1.02 if DSB_model and aply_DSB_model else 1
            ...
            if DSB_model and aply_DSB_model:
...
if __name__ == '__main__':
    ...
    # dsb_model/aply_DSB_model to test
    methods = [ 
        (False, None),          # NR
        (True, False),          # NR DS DSB_model=False
        (True, True),           # NR DS DSB_model=True
    ]
